...
#include "node.h"
#include "hashtable.h"
void* TraverseTree(Node *HEAD, short int *tree_size, unsigned char *prefix, Hashtable *ht)//compr
{
    if(HEAD != NULL)
    {
    	...
    	*tree_size +=1;
        TraverseTree(HEAD->left, tree_size, Concat(prefix, '0'), ht);
        TraverseTree(HEAD->right, tree_size , Concat(prefix, '1'), ht);
    }
    return ht;
}
...
void print_pre_order(Node *tree)
{
	...
	item = tree->item;
	freq = tree->freq;
	if(tree->left != NULL)print_pre_order(tree->left);
	if(tree->right != NULL)print_pre_order(tree->right);
    }
}
